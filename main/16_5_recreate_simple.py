...
def calculate_sdf_at_points(
    mesh_encoder, sdf_calculator: SDFCalculator, vertices_tensor, time_index_animation, query_points_np
) -> np.ndarray:
    """
    ...
    vertices = vertices_tensor[time_index_animation].view(1, -1)  # Flatten the vertices
...
def visualize_mesh_and_points(faces, vertices, b_min, b_max, query_points):
    """
    Visualize a mesh, its bounding box, and a set of query points.

    Args:
        faces (np.ndarray): Faces of the mesh, shape (N, 3).
        vertices (np.ndarray): Vertices of the mesh, shape (M, 3).
        b_min (np.ndarray): Minimum coordinates of the bounding box [x_min, y_min, z_min].
        b_max (np.ndarray): Maximum coordinates of the bounding box [x_max, y_max, z_max].
        query_points (np.ndarray): Query points to visualize, shape (K, 3).
    """
    # Convert faces to PyVista-compatible format
    pyvista_faces = np.hstack([np.full((faces.shape[0], 1), 3), faces]).flatten()
    ...
    # Create the bounding box as a PyVista cube
    bounding_box = pv.Box(bounds=(b_min[0], b_max[0], b_min[1], b_max[1], b_min[2], b_max[2]))

    # Create the query points as a PyVista point cloud
    query_points_cloud = pv.PolyData(query_points)
    ...
    # Add the mesh to the plotter
    plotter.add_mesh(mesh, color="lightblue", show_edges=True, opacity=0.7, label="Mesh")

    # Add the bounding box to the plotter
    plotter.add_mesh(bounding_box, color="green", opacity=0.3, label="Bounding Box")

    # Add the query points to the plotter
    plotter.add_points(query_points_cloud, color="red", point_size=4, label="Query Points")

    # Add a legend and set the background color
    plotter.add_legend()
    plotter.set_background("white")
